...
step by step. The short form manual is available on \xcsoarwebsite{/
...
\begin{itemize}
\item Android mobile phones and tablets with Android 6.0 or newer 
\item eReader Kobo
...
On iOS, only Bluetooth Low Energy serial adapters can be used (Apple
does not offer classic Bluetooth serial connections to third-party
apps).  XCSoar supports adapters implementing the Nordic UART
Service, the Microchip/ISSC transparent UART or the HM-10 profile;
they appear as ``BLE port'' in the port list.
...
directory (\xcsoarwebsite{/hardware/}). Unplug the Kobo and reboot it (switch it off completely,
...
\begin{itemize}
\item \textbf{64-bit OpenGL} (target ``WIN64OPENGL''): the recommended
  version. Run the installer, or use the \verb|.zip| file for a manual
  install. Supports high-DPI displays, GPU acceleration, and matches the
  Unix and Android versions in features and visuals.
\item \textbf{32-bit OpenGL} (target ``WIN32OPENGL''): same as the 64-bit
  OpenGL build for 32-bit Windows; installer or \verb|.zip| available.
\item \textbf{64-bit legacy GDI} (target ``WIN64''): \emph{deprecated.}
  Download \verb|XCSoar.exe| and place it on your hard disk; no installer.
  This single-file build works on very old computers without GPU
  acceleration, but will be removed in a future release; use
  ``WIN64OPENGL'' instead.
\item \textbf{32-bit legacy GDI} (target ``PC''): \emph{deprecated.}
  Download \verb|XCSoar.exe| and place it on your hard disk; no installer.
  Will be removed in a future release; use ``WIN32OPENGL'' instead.
\end{itemize}
...
\texttt{-h} and \texttt{--help}).
\texttt{-h}/\texttt{--help} prints the option list and exits;
\texttt{-version}/\texttt{--version} prints version and licence text and exits.
Invalid options print a brief usage reminder on standard error.
...
Useful options include \verb|-datapath=...| for the data directory,
\verb|-profile=...| to load a profile file, \verb|-simulator| or \verb|-fly|
when available to skip the startup mode prompt, and geometry switches such as
\verb|-portrait|, \verb|-square|, and \verb|-small|.
On desktop Unix/macOS builds, \verb|-replay=PATH| may load flight replay data.
